...
if (isset($_GET['filter']) && $_GET['filter'] != "")
{
    ...
    $queryEndpointsSQLDomainTotalRecords = "SELECT agent, heartbeat, NOW(), system, version, status, domain, ipaddress, name, ruleset, gender, SUM(totalwords) AS totalwords, SUM(pressure) AS pressure, SUM(opportunity) AS opportunity, SUM(rationalization) AS rationalization, COUNT(agent) AS sessions FROM (SELECT SUBSTRING_INDEX(agent, '_', 1) AS agent, heartbeat, NOW(), system, version, status, domain, ipaddress, name, ruleset, gender, totalwords, pressure, opportunity, rationalization FROM t_agents GROUP BY agent ORDER BY heartbeat DESC) AS agents WHERE domain='".$session->domain."' OR domain='thefraudexplorer.com' GROUP BY agent HAVING agent LIKE '".$searchString."%' OR name LIKE '".$searchString."%' OR domain LIKE '".$searchString."%' ORDER BY ".$sortColumnSelected." ".$sortOrderSelected."";
    ...
    $queryEndpointsSQLDomain_wOSamplerTotalRecords = "SELECT agent, heartbeat, NOW(), system, version, status, domain, ipaddress, name, ruleset, gender, SUM(totalwords) AS totalwords, SUM(pressure) AS pressure, SUM(opportunity) AS opportunity, SUM(rationalization) AS rationalization, COUNT(agent) AS sessions FROM (SELECT SUBSTRING_INDEX(agent, '_', 1) AS agent, heartbeat, NOW(), system, version, status, domain, ipaddress, name, ruleset, gender, totalwords, pressure, opportunity, rationalization FROM t_agents GROUP BY agent ORDER BY heartbeat DESC) AS agents WHERE domain='".$session->domain."' GROUP BY agent HAVING agent LIKE '".$searchString."%' OR name LIKE '".$searchString."%' OR domain LIKE '".$searchString."%' ORDER BY ".$sortColumnSelected." ".$sortOrderSelected."";
